...
describe('account-profile（账号↔profile 映射，用户隔离方案A）', () => {
  ...
  afterEach(() => {
    _resetAccountProfileMapForTest()
    $auth.set({ token: null, username: null, isCustomKey: false, allowModelSelect: true, platformModels: [], mode: 'trial', score: 0, loginAt: null, apiKey: null, avatar: null })
  })
  ...
  it('accountLockedProfile：登录账号已登记返回其 profile key', () => {
    ...
    $auth.set({ token: 't', username: '13800138000', isCustomKey: false, allowModelSelect: true, platformModels: [], mode: 'trial', score: 0, loginAt: Date.now(), apiKey: null, avatar: null })

    expect(accountLockedProfile()).toBe('default')
  })

  it('accountLockedProfile：登录账号未登记返回 null（老 token 未走新登录流）', () => {
    $auth.set({ token: 't', username: 'ghost', isCustomKey: false, allowModelSelect: true, platformModels: [], mode: 'trial', score: 0, loginAt: Date.now(), apiKey: null, avatar: null })
