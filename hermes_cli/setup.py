"""
Interactive setup wizard for 奇计 Agent.
...
_DOCS_BASE = "https://www.aicps.vip/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ 奇计 Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure 奇计 using environment variables or config commands:")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the 奇计 auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where 奇计 runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where 奇计 delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects 奇计 to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://www.aicps.vip/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://www.aicps.vip/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with 奇计 from anywhere.")
...
# Config values may have different semantics between OpenClaw and 奇计.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure 奇计 to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point 奇计 at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point 奇计 at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point 奇计 at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point 奇计 at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to 奇计 equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing 奇计 config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in 奇计.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ 奇计's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("奇计 can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing 奇计 configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing 奇计 config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in 奇计 (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    奇计 session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ 奇计 Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print()
    print_info("  Sign up: https://www.aicps.vip/manage-subscription")
    print()
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ 奇计 Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ 奇计 Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your 奇计 Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have 奇计 configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up 奇计?",
            [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Qiji to messaging apps to chat from anywhere.")
